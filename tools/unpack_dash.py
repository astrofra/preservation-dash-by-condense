#!/usr/bin/env python3
"""Extract Dash.GCR using the metadata table preserved in Dash.exe.

Python 3.10+ and the sibling unpack_klx.py module are required; there are no
third-party dependencies. See documentation/dash-container-format.md.
"""
...
def read_entries(
    ...
    if len(table) < ENTRY_COUNT * 12:
        raise FormatError("truncated Dash metadata table")
    entries = []
    record_offset = 0
    total_size = 0
    seen: set[str] = set()
...
def main() -> int:
    ...
    parser.add_argument("executable", type=Path, help="matching preserved Dash.exe")
    parser.add_argument("archive", type=Path, help="matching preserved Dash.GCR")
